...
if (!file_exists("nfo/{$id}.cap")) {
	...
	$links = implode(" / ",$links);
        ...
        $links .= isset($info['UPLOADER']) ? " - Uploaded by {$info['UPLOADER']}" : "";

	$info['info'] = <<< LALALALA
<TEXTFORMAT LEADING="2"><P ALIGN="LEFT"><FONT FACE="Impact" SIZE="16" COLOR="#FFFFFF" LETTERSPACING="0" KERNING="0">{$info['TITLE']}\n<FONT SIZE="12">{$info['BY']}\n<FONT SIZE="10">{$links}</FONT></FONT></FONT></P></TEXTFORMAT>
LALALALA;
	//$info['time'] = $getid3;
}
